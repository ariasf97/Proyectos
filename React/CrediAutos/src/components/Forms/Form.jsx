...
export function Form({ isActive }) {
    ...
    const utmGclid = UTMS.gclid
    const onSubmit = async (formData) => {
        const token = await generarToken('rdflorez', 'pruebaetb');
        toast.promise(
            realizarPeticionPost(utmSource, utmMedium, utmCampaign, utmGclid,  formData.telefono, formData.terminos, token),
            {
                success: (success) => {
                ...
                error: (error) => {
                    if (error.response && error.response.status === 405) {
                        return { title: 'Registro Duplicado', description: 'Ya se ha registrado en los últimos 30 minutos. Por favor, intente más tarde.' };
                    }
                    console.log(error)
                    return { title: 'Ha ocurrido un error', description: 'Intente de nuevo más tarde.' };

                },
                loading: { title: 'Se están enviando los datos', description: 'Espere un momento, por favor.' },
            }
        );
    };
